  ...
  return (
    <div className="w-full px-2 sm:px-0">
              ...
              filteredItems.map((item: MarketplaceItem) => (
                <div key={item.id} className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                  ...
                  <div className="flex flex-wrap justify-end gap-2">
                    ...
                    {(item.approved || !item.approved) && !item.sold && (
                      <Button 
                        variant="outline" 
                        size="sm"
                        className="border-red-500 hover:bg-red-500 hover:text-white"
      ...
      <AlertDialog open={!!itemToDelete} onOpenChange={(open) => !open && setItemToDelete(null)}>
        <AlertDialogContent className="w-[95%] max-w-md z-[110]">
          <AlertDialogHeader>
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="w-[95%] max-w-md max-h-[85vh] overflow-y-auto z-[100]">
          {selectedItem && (
            <>
              <DialogHeader>
